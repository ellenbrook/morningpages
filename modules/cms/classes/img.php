...
abstract class img extends Kohana_Image {
	
	public static function show($filename, $width = NULL, $height = NULL, $master = NULL)
	{
		...
		if(($width && $width != $img->width) || ($height && $height != $img->height))
		{
			...
			$newname		= implode('.',array_slice($filenameparts,0,count($filenameparts)-1)) . $img->width . 'x' . $img->height . '.' . self::imgtype_to_ext($img->type);
	...
	public static function imgtype_to_ext($type)
	{
		$imgtypes = array(
			1 => 'gif',
			2 => 'jpg',
	...
	public static function _get($filepath, $width = false, $height = false, $crop = false, $dir = 'media/cache/')
	{
		$imgtypes = array(
			1 => 'gif',
			2 => 'jpg',
